...
- **Base URL：** `http://10.200.38.35:8000/api/v1/external`
- **認證方式：** API Key（`X-API-Key` Header）
- **輸出格式：** JSON / CSV
- **Swagger UI：** `http://10.200.38.35:8000/api/v1/external/docs?token={API_KEY}`
  （瀏覽器開文件頁沒辦法帶 Header，所以用 `?token=`；程式呼叫請用 Header）

> `http://10.200.38.35:8000/docs` 是**內部**完整文件，需管理者登入，對外不提供。
...
（分鐘剛開始時 `elapsed_sec` 很小，外推值抖動大，建議等 20 秒以上再顯示）。
...
以 `interval=1m` 搭配 20 秒輪詢為例，實際節奏是：每分鐘產生一個新桶，
桶結束後約 15 秒可取得；三次輪詢中約有一次拿到新桶，其餘為同一份資料
（這是桶大小決定的，不是延遲）。若希望每次輪詢都有新資料，
...
輪詢頻率 20 秒沒有問題（實測單次回應約 20～40 毫秒）。速率限制預設 120 次/分。
...
curl -H "X-API-Key: {Key}" \
  "http://10.200.38.35:8000/api/v1/external/vd-report?start_time=2026-04-07T00:00:00%2B08:00&end_time=2026-04-07T12:00:00%2B08:00&interval=5m"
```
...
```bash
curl -H "X-API-Key: {Key}" \
  "http://10.200.38.35:8000/api/v1/external/congestion-report?start_time=2026-04-07T00:00:00%2B08:00&end_time=2026-04-07T12:00:00%2B08:00"
```
    ...
    "streams": [
      {
        ...
        "fps": 30.0,
        "urls": {
          "rtsp":  "rtsp://10.200.38.35:8554/cam_2",
          "hls":   "http://10.200.38.35:1984/api/stream.m3u8?src=cam_2",
          "mjpeg": "http://10.200.38.35:1984/api/stream.mjpeg?src=cam_2",
          "webrtc_signal": "http://10.200.38.35:1984/api/ws?src=cam_2"
        }
      }
    ]
  }
...
API_KEY = "{Key}"
BASE = "http://10.200.38.35:8000/api/v1/external"
...
# VD 報表 JSON
curl -H "X-API-Key: {Key}" \
  "http://10.200.38.35:8000/api/v1/external/vd-report?start_time=2026-04-07T00:00:00%2B08:00&end_time=2026-04-07T12:00:00%2B08:00"

# VD 報表 CSV 下載
curl -H "X-API-Key: {Key}" -o vd_report.csv \
  "http://10.200.38.35:8000/api/v1/external/vd-report?start_time=2026-04-07T00:00:00%2B08:00&end_time=2026-04-07T12:00:00%2B08:00&format=csv"
