...
| (B) 操作模式 ⚪ | tM-PD3R3 **DO2** → 12V 白燈（與控制模式共用） | DO2 | ✅ **恆亮=手動 或 下載中、滅=自動(遠端)+閒置** |

**DO2 雙功能說明**：同一顆白燈同時表示「控制模式（自動/手動）」與「遠端下載中」狀態——  
`白燈 ON` = 手動模式 **或** 下載進行中；`白燈 OFF` = 自動(遠端)模式且非下載中（且無通訊故障）。  
**設計理念**：自動(遠端)模式為預設運行狀態 → 燈號最少 (只剩綠燈)；切到手動或下載中時白燈點亮提醒操作者。  
...
| 4 | 操作模式／下載中 | ⚪ 白 | DO2 | **恆亮=手動模式 或 下載中；滅=自動(遠端)+閒置** | — |
