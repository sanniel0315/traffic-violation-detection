    ...
    def _process_post_yolo(infer_frame, detections, cur_ts):
        ...
        if _mode_traffic and (vehicles or _vehicles_all) and det_zones:
            ...
            try:
                ...
                #    構想是:某個 track **第一次被看到**就已經深在框內(離邊界超過
                #    這個像素數)時,只標記成「在框內」不計流量。
                #
                #    🛑 2026-09-12 實測後**預設關閉**(改為 0)。原因:遠處的車太小、
                #       偵測不到,要駛近到一定距離才第一次被偵測到 —— 而那個位置
                #       本來就在框內。所以「第一次出現就深在框內」並不是 track 斷掉
                #       的可靠訊號,對這幾支相機而言更像是「正常的遠處進場」。
                #       開啟後四個測點全部掉 27~54%(連短框的上游測點 NE-1 都掉 54%),
                #       而且上匝道上下游比值由 0.80 變成 1.28 —— 離 1 更遠,是變差。
                #       程式留著、參數可調,但要用之前必須先在該站量過再決定門檻。
                _FLOW_ENTRY_EDGE_PX = float(os.getenv("SIGNAL_FLOW_ENTRY_EDGE_PX", "0") or 0)
