...
def _rewrite_html(body: bytes, host: str, web_port: int) -> bytes:
    ...
    #    http://10.200.38.35/moxa/home.htm 這種。只改 / 開頭的不夠,遠端瀏覽器
...
async def proxy_web(host: str, web_port: int, path: str, request: Request,
    ...
    for k in _FORWARD_RES_HEADERS:
        ...
        if k == "location":
            ...
            # (MiiNePort 實測回 http://10.200.38.35/moxa/home.htm),兩種都要處理,
...
#    2026-08-17 現場實例:NPort 的 IP 被改成 10.200.38.35(遮罩 /20、閘道
#    10.200.32.254),但網線還留在 192.168.1.x。它收到 ping 之後要回覆,會查
#    自己的路由表 → 送去 10.200.32.254 → 那個閘道不在這個廣播域 → 回不來。
