...
來源:號誌控制器 RS-232 → Moxa MiiNePort E1 → TCP。現場實測參數:
    IP 10.200.40.222  TCPDataPort 1001   Mode TCP / Role TCP Server   MaxConnect 1
    (2026-08-21 MiiNePort 由 10.200.38.35 移到 10.200.40.222;10.200.38.35 已改給分析器主機)
...
SIGNAL_HOST = os.getenv("SIGNAL_TC3_HOST", "10.200.40.222")
...
# 🛑 為什麼需要:號誌來源是 MiiNePort(現場 10.200.40.222)。MaxConnect=1、又只
#    此一條線,對端靜默或被交控中心搶走時,TCP 連得上卻永遠沒有資料 —— 沒有這
#    道檢查就會傻等到天亮。(2026-08-21 前 NPort 曾在 10.200.38.35 且網線落在
...
# ── 中間層對控制器回 ACK ──────────────────────────────────────────────
# 🛑 2026-09-17 實測證實的因果。控制器送出的每一種訊框都在 2.0~2.1 秒後**整批**
#    重來、最多 9 次(09-15 起重複率 81%)。開啟我方回 0F80 的 20 分鐘測試窗後:
#      我方有回的  5F00 78%→0%、5F03 72%→13%、0F80 74%→10%
#      我方沒回的  0FC1 89%→61%、5FC8 84%→65%
#    → 控制器完全照協定在做:等對方確認,沒等到就重送。缺的是**收訊端的 ACK**,
#      而中央端歷來一則 0F80 都沒送過。
#
# 🛑 為什麼由我方回:我方是中間層 —— 控制器的訊框先進到我們這裡,再由我們上傳中央;
#    下行的中央命令也由我們轉給控制器。對控制器來說,收訊端就是我們,我們回 ACK
#    是據實回應「已收到」,不是代替中央承諾什麼。
#    這與「不偽造 ACK 給中央」是兩回事:那條講的是**不要替控制器回答中央**,
#    因為我方沒有權限代表控制器接受命令;這裡方向相反,且我方確實收到了。
#
# 🛑 永遠不回 0F80/0F81 本身 —— 會變成互相回應的無窮迴圈。
...
def _ack_controller_frame(rec: dict) -> None:
    ...
    try:
        ...
        _enqueue_frame({"ts": time.time(), "src": "self", "code": "0F80",
                        ...
                        "raw": frame.hex(" ").upper(), "user": "relay-ack"})
    except Exception as exc:
...
#   1 / 2 → 下匝道、3 / 5 / 6 → 上匝道
# 🛑 只綁匝道、不綁分相編號(編號會因現場改線路對調;分相一律用 phase_of_role 推)。
# 🛑 2026-09-08 使用者現場更正:原本編號 4 的那座實際是 **6**,而且屬上匝道,
#    不是原記載的下匝道。編號因此是 1/2/3/5/6(沒有 4),總數仍是 5 座。
