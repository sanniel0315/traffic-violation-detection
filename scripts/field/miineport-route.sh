#!/bin/sh
# MiiNePort(10.200.38.35)單播繞道 —— ⚠️ 2026-08-18 起「預設不需要、也不該啟用」。
#
# 這支腳本是為了 2026-08-17 當下的現場接線寫的:號誌那顆 MiiNePort 掛在
# enP5p4s0(192.168.1.x)這一段,但它自己的 IP/遮罩/閘道是 10.200.38.35 /20 →
# 10.200.32.254,回覆封包會查自己的路由表送去那個不在本廣播域的閘道而回不來。
# 解法是在 enP5p4s0 補一個 10.200.38.200/32 的來源位址 + /32 主機路由 + 靜態 ARP。
#
# 🛑 2026-08-18 現場把號誌那條線改接進 10.200.32.0/20(跟相機同一段,走 enP5p5s0)。
#    此後這支腳本從「救援」變成「破壞」—— 它每 30 秒會:
#      ① 把 10.200.38.35/32 釘回 enP5p4s0(那一段已經沒有這台設備)
#      ② 灌 nud permanent 的靜態 ARP(設備不在,封包石沉大海也不會報錯)
#      ③ ip neigh del 10.200.38.35 dev enP5p5s0 —— 主動刪掉「正確那一側」的 ARP
#    症狀:ping 100% 丟包、抄錄器一直 TimeoutError,但 link/路由/ARP 表看起來都「正常」。
#    當天實際查法:Moxa UDP 4800 廣播探測分別綁兩個介面 ——
#      enP5p4s0 找到 0 台 / enP5p5s0 找到 69 台(含 00:90:e8:89:11:42 = 10.200.38.35)。
...
while :; do
    ...
    # 避免核心挑到那條。不影響相機本身(10.200.40.22~25 各自的 ARP 不動)。
