...
- nmcli 連線 `eth1-test` 已配完整（**不用再下任何設定命令**）：
  - IP `10.200.38.35/20`、閘道 `10.200.32.254`、`ipv4.route-metric 4000`（實際 default metric 顯示 24000，高於主口 100 → 平時主口優先、eth1 備援不搶路由）。
- **現場上線步驟**：
  1. eth1(`enP5p3s0`) 接現場 10.42.x 網路
  2. 拔線或 `sudo nmcli con down <enP5p5s0 連線>` 停掉測試主口 → eth1 自動變唯一預設路由
  3. 驗證：`ping 10.200.32.254` + 相機/服務正常；此後管理走 `10.200.38.35`
- 測試環境 `ping 10.200.32.254` 不通是正常（eth1 沒接到現場網段）。
