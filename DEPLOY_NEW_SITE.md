...
```
ipv4.addresses   10.200.38.35/20          遮罩 255.255.240.0
ipv4.routes      10.0.0.0/8 → 10.200.32.254
...
```bash
ip route get 10.200.32.254   # → dev enP5p5s0
ip route get 10.201.4.123    # → dev enP5p5s0 via 10.200.32.254
ip route get 8.8.8.8        # → dev enP5p4s0 via <4G 閘道>
```

`10.200.32.0/20` 內是直連；範圍外的 `10.x` 靠 `10.0.0.0/8` 那條走現場閘道。
若現場有非 `10.x` 的網段，要另外加路由。

> 未接到現場網路時 `ping 10.200.32.254` 不通是正常的（ARP 顯示 `INCOMPLETE`），
...
```env
STREAM_HOST=10.200.38.35
```

`.env` 有 gitignore，`git reset --hard` 不會動它，設一次即可。
驗證：`curl -H "X-API-Key: <key>" http://10.200.38.35:8000/api/v1/external/streams`
回傳的 `host` 應為 `10.200.38.35`。
...
| 站點 | NTP 位址 |
|---|---|
| R34 動態號誌 | `10.202.0.111` |

新站台一次帶進去：

```bash
FIELD_NTP=10.202.0.111 TRAFFIC_STORAGE_ROOT=/mnt/nvme/traffic bash scripts/setup_new_site.sh
```
...
sed "s|__FIELD_NTP__|10.202.0.111|" deploy/timesyncd/field-ntp.conf.template \
...
- 本機 Tailscale：機器名 `<machine-name>`、tailnet IP **`<jetson-tailscale-ip>`**、tailnet 帳號 `<tailnet-account>`、已開 `--ssh`、`tailscaled` 開機自啟+自動重連。
- **系統登入用戶是 `ubuntu`**；`<tailnet-account>` 只是 Tailscale 帳號，別拿來當 ssh user。
- 連法（你的電腦先裝 Tailscale 並登入同帳號）：
  - `ssh ubuntu@<jetson-tailscale-ip>`，或 MagicDNS `ssh ubuntu@field-jetson`
  - Cowork / Claude Code 桌面版：SSH host 填 `ubuntu@<jetson-tailscale-ip>` 或 `ubuntu@field-jetson`
- **關鍵特性**：Tailscale 是疊在底層網路上的 overlay，兩端不用同網段、你的電腦也不用能直連現場 10.42.x；只要**現場那條 eth port 能通外網**，主機就自動回 tailnet，`<jetson-tailscale-ip>` 就連得進（不隨底層 IP 變）。純內網不通外網才需改插行動網卡。
...
- **現場上線確認流程**：手機收到 ntfy 上線通知 → `ssh ubuntu@<jetson-tailscale-ip> hostname`（應回 `nvagxorinafer750a1`）→ 通了代表遠端運維正常。離線時連不上是正常，不是故障。
