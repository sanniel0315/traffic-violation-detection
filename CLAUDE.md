...
- `docs/遠端管理功能_條文對照.md` — 驗收條文 A~J 對 TC3 碼的逐項實測狀態。🛑 三則查詢被這台控制器**拒收**(`5F4A`/`5F4B` ErrorCode=08、`5F5F` ErrorCode=01),在問到廠商之前條文 F 排程與 H 步階排列**不可宣稱做得到**;J 的 NTP 是「我方對 NTP 再寫給控制器」,不是控制器直連
