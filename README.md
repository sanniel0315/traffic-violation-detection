...
> （`.108` → `.102` → `192.168.200.87` → `192.168.1.3`）。
> 一律走 Tailscale 或 Cloudflare Tunnel。

```bash
# 從 dev PC ssh 進 Jetson（Tailscale 固定位址）
ssh ubuntu@<jetson-tailscale-ip> "cd ~/traffic-violation-detection && git pull origin main && sudo systemctl restart traffic-api"

# 驗證部署版本
ssh ubuntu@<jetson-tailscale-ip> "cd ~/traffic-violation-detection && git log --oneline -1"

# 外網（固定網址）
curl https://<public-api-host>/api/health
```
