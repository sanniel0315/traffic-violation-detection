...
⚠️ **本引擎目前與 OPAC 的一致率只有 42%（40 筆實測），尚不可用於接管。**
   ...
   **2026-09-02 追加:400 筆連續序列分析(30 個完整綠燈段)，結論如下。**

   ✅ 已確定(可直接用):
     - `SWITCH ⟺ pn1 > pn2` —— **400 筆 399 命中(100%)**，規則本身無疑義
     - `pn1 - pn2` 值域封閉在 −4..+3；SWITCH 全部落在 ≥ +1，KEEP 落在 ≤ 0
     - `pn1 ≈ swlRed + faRed + saRed` —— 400 筆命中 356(89%)，不吻合的 44 筆
       差值恆為正(+1~+3)且與綠側量相關 → pn1/pn2 互相耦合，非獨立計算

   ❌ 已排除(實測命中率，皆不可用):
     瞬時式  faG+saG 32% / swlG+faG+saG 9% / faG+saG+cl 16% / faG+saG+el−cl 21%
     耦合式  pn1+faG+saG−紅側總量 33%
     遞迴式  pn2[t-1]+faG+saG−cl 18% / −el 17% / max(0,…) 26~29%
             pn1[t-1]+faR+saR 58%(最佳，仍不足)

   **結論:pn1/pn2 是 OPAC 內部有界狀態機的兩個耦合計數器，
   無法從 decision.log 的外部欄位重建。** 已測 15 種假設全部失敗，
   再猜下去沒有意義 —— 缺的是狀態機的轉移規則本身，那只在 OPAC 原始碼裡。

   **要 bypass 的兩條實務路徑**:
   (a) **不複製 OPAC，改用我方自己的決策法則** —— 規則骨架(pn1>pn2 比較、
       min/max green、主線保護)已驗證可用，把 pn1/pn2 換成我方定義的
       延滯成本(例如直接用排隊公尺與等待時間)，再與 OPAC 比「結果好壞」
       而非「逐筆一致」。這條完全可行，且不依賴中心端。
   (b) 向中心端取得 OPAC 演算法文件/原始碼，完全複製其狀態機。

   在其中之一完成前，本引擎只能當「觀察與比對工具」，不可作為接管依據。
