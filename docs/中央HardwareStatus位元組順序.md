# 中央端 HardwareStatus 位元組順序不一致（含 swap 補償）

**站點**：國道8號新市交流道（東向） **日期**：2026-09-08
**狀態**：🛑 **2026-09-09 14:48 改定案 `hwstatus-mode = raw` ＋ 遮蔽 bit13**
（改選「燈號綠」那一邊；代價是告警欄有假的 `SIGNAL_DRIVER_UNIT_ERROR`）

> 2026-09-08 的 `swap` 定案已作廢。改變的理由不是偏好，是**證據變了**：
> 09-09 完成 16 位逐位實測，那條告警確定是中央讀法造成的假故障，
> 有完整對照表可證（`docs/HardwareStatus_逐位實測紀錄.md`）。
> 使用者 09-09 決定：「測完要給綠燈的封包」。

---

## 一句話

中央端**用兩種不同的位元組順序在讀同一個 2-byte 欄位**：
判斷「正常/故障」時正常讀，產生告警名稱時反著讀。

因此**沒有任何一個值能同時讓兩者都正確** —— 這是中央端的實作矛盾，
我方無法用任何設定繞過。控制器本身沒有任何硬體異常。

---

## 🛑 核心發現：中央有兩條解析路徑，順序不一致

| 路徑 | 中央的讀法 | 依據的位元 |
|---|---|---|
| **就緒旗標**（正常/故障燈號） | **正常讀**（big-endian） | bit14 `controllerReady` |
| **告警名稱**（`eq_hw_status`） | **反著讀**（byte-swapped） | 反讀後的位元對應錯誤表 |

三組實測、三種不同的顯示結果，全部吻合，沒有一次需要另外找理由：

| 我方送 | bit14（as-sent） | 就緒旗標 | 告警名稱（反讀） | 中央實際顯示 |
|---|---|---|---|---|
| `0x4000` | 1 | 就緒 → **不故障** | `0x0040` → bit6 | `SIGNAL_DRIVER_UNIT_ERROR` |
| `0x0040` | 0 | **故障** | `0x4000` → bit14（錯誤表無此項） | **故障，沒代碼** |
| `0x0020` | 0 | **故障** | `0x2000` → bit13 | **`TIMMING_PLAN_ON_TRANSITION`** |

### 🛑 2026-09-09 更新：不只是順序，是**同一個實體位元**

逐位實測（`docs/HardwareStatus_逐位實測紀錄.md`）之後，這件事可以講得更死：

```
封包 offset 9 這一個位元組:   0 1 0 0 0 0 0 0     ← 值為 0x40 時
                                 ↑
              我方叫它 bit14 controllerReady（它是我方高位元組的 bit6）
              中央叫它 bit6  signalDriverUnitError（中央把這個位元組當 bit0~7）
```

**就緒旗標與 `SIGNAL_DRIVER_UNIT_ERROR` 讀的是同一個位元。**
不是找不到對的值，是**這一個位元不可能同時是 1 和 0**。
四個角 2026-09-09 全部由現場畫面確認：

| offset 9 的 bit6 | 硬體狀態燈 | 告警欄 | 確認時間 |
|---|---|---|---|
| **1**（送 `40 00`） | **綠色／正常** | `SIGNAL_DRIVER_UNIT_ERROR` | 13:58 / 14:01 |
| **0**（送 `00 40`） | **紅色／異常** | 空 | 13:38 起 |

`0x4040` 也救不了：告警只看 offset 9 那個位元組，它照樣是
`SIGNAL_DRIVER_UNIT_ERROR`。（原本以為 `0x4040` 是「可行但不誠實」的第三條路，
實測之後連可行都不是。）

### 為什麼我方無解

要讓就緒旗標正常，送出的值**必須含 bit14**（`0x4000`，高位元組）。
而 bit14 反讀後**必定**落在 bit6，那在告警表裡就是 `SIGNAL_DRIVER_UNIT_ERROR`。

**位元組順序決定了這兩件事不可能同時成立**，不是調參數能繞過的。

這也解釋了 2026-09-07 那個一直對不上的舊實測：當時送 `0x4000` 中央顯示「正常」——
那是**就緒旗標路徑**（正常讀 bit14）給的結論，與告警名稱路徑無關。

### 定案：`raw` ＋ 遮蔽 bit13（2026-09-09 14:48 起）

兩條路徑無法同時滿足，只能選承受哪一種：

| 模式 | 中央「硬體狀態」欄 | 中央 `eq_hw_status` |
|---|---|---|
| **`raw`（現行）** | **正常／綠燈** | `SIGNAL_DRIVER_UNIT_ERROR`（假的，已報備） |
| `swap`（2026-09-08 曾短暫採用，已作廢） | 故障（且無代碼） | 空 |

🛑 下面這一段保留的是 09-08 當天選 `swap` 的理由，**結論已被 09-09 推翻**
（使用者指示「測完要給綠燈的封包」）。留著是為了說明兩種取捨各自的代價，
不是現行組態。

當日兩邊都試過、也都被退回：18:26 切 `raw`（現場：「硬體狀態顯示故障」→
其實是前一刻 swap 的殘留）、18:40 切回 `swap`（現場：「這個 ERROR 要拿掉」）。
09-08 當下選 `swap`：告警欄有代碼、會進中央的告警清單、會被當成真故障處理，
影響比一個沒有代碼的燈號大。
**隔日 09-09 逐位實測完成後改選 `raw`** —— 因為已能證明那條告警是中央讀法
造成的假故障，可以連同對照表一起報備，而「硬體狀態燈號紅」沒有辦法報備掉。

🛑 這兩個狀態**互斥**。往後任何一方再抱怨，先看這張表再動手，
不要又切一次 —— 切過去就是換成另一種抱怨。

#### 🛑 本文件曾寫錯，記在這裡避免重演

2026-09-08 18:01 一度改用 `swap`，並寫下「`swap` 在 XML 上完全乾淨，
**沒有代價**」。**那句是錯的。**

錯誤原因：拿 `scmXMLData.xmL` 的內容去否定一個關於「硬體狀態燈號」的結論，
但**那份 XML 只有 `eq_hw_status`，根本不包含硬體狀態那個欄位**。
看到部分資料就推翻整體模型。

實際情形是：`swap` 時 `eq_hw_status` 確實變空，但中央畫面的「硬體狀態」
同時變成**故障**（就緒旗標路徑正常讀 bit14，而 `swap` 送出的 `0x0040`
沒有 bit14）。現場當天兩次回報正好各對應一種模式：
`swap` 時說「有故障但沒代碼」、`raw` 時貼 `SIGNAL_DRIVER_UNIT_ERROR` ——
兩者是同一個矛盾的兩面，不是兩個問題。

**教訓**：用一份資料推翻結論之前，先確認那份資料**包含**該結論講的欄位。

#### 為什麼沒有第三種選擇

唯一能讓兩條路徑都看到 bit14 的值是 `0x4040`（位元組對稱）。但那等於
**我方主動送出一個「號誌驅動單元錯誤」的位元** —— 對主管機關謊報故障情形，
本專案不做，程式的遮蔽白名單也擋著。

正解仍是中央端讓兩條路徑用同一種位元組順序（依協定應為 big-endian）。

---

## 中央看到的假故障

| 中央顯示 | 實際狀態 | 佐證 |
|---|---|---|
| `記憶體異常` | 機箱後門開啟 | 我方電子鎖門磁實測（後門斷開、前門閉合） |
| `I/O unit error` | 外部時相控制進行中 | 演算法正常運作，當日下發 53 次全數被接受 |
| `SIGNAL_DRIVER_UNIT_ERROR` | 控制器就緒 | 號誌運轉正常，此旗標恆為 1 |

## 位元對照

| 我方位元 | 我方語意 | 中央讀成 | 中央顯示 |
|---|---|---|---|
| bit9 | 機箱門開啟 | bit1 | 記憶體異常 |
| bit13 | 外部時相控制進行中 | bit5 | I/O unit error |
| bit14 | 控制器就緒 | bit6 | SIGNAL_DRIVER_UNIT_ERROR |
| bit5 | I/O 單元錯誤 | bit13 | TIMING_PLAN_ON_TRANSITION |

## 四組實測（`force` 模式逐段送出，現場同步記錄中央畫面）

| 時間 | 我方送 | 反讀為 | 中央實際顯示 | |
|---|---|---|---|---|
| 10:04–10:07 | `0x6000` | `0x0060` | IO_UNIT_ERROR、SIGNAL_DRIVER_UNIT_ERROR | ✅ |
| 10:07–10:10 | `0x4020` | `0x2040` | SIGNAL_DRIVER_UNIT_ERROR、TIMING_PLAN_ON_TRANSITION | ✅ |
| 12:15 | `0x4200` | `0x0042` | 記憶體異常、SIGNAL_DRIVER_UNIT_ERROR | ✅ |
| 12:43 | `0x6200` | `0x0062` | 記憶體異常、I/O unit error、SIGNAL_DRIVER_UNIT_ERROR | ✅ |
| **16:58** | **`0x4000`** | **`0x0040`** | **SIGNAL_DRIVER_UNIT_ERROR** | ✅ |

五組值、五次預測全中。

### 2026-09-08 17:00 複驗：中央**仍然**反著讀

當日曾短暫切到 `raw`，測中央是否已修正。結果中央顯示
`SIGNAL_DRIVER_UNIT_ERROR` —— 正是 `0x4000` 反讀成 `0x0040`（bit6）的結果。
**中央端未修正，`swap` 必須留著。** 17:02:53 已切回。

> 先前推測「中央可能已修好」的旁證（15:38 起不再送 `5F10 01 00`）**不成立**，
> 那與位元組順序無關。

## 我方為 big-endian 的依據

- **bit14（controllerReady）在運轉中恆為 1。** 照中央的讀法，等同這台控制器
  連續十天都有「號誌驅動單元錯誤」，而同期運轉完全正常、無維修紀錄。
- **bit13 與外部控制授權高度相關。** 與我方送出的 `5F10` 控制策略 bit4 逐筆比對，
  **45,843 筆**樣本一致率 **95.8%**，不一致的部分全是回報週期造成的延遲
  （策略設定後 bit13 跟上中位 14.5 秒）。此相關性在反讀之下不存在。
- **同協定其他 2-byte 欄位皆 big-endian**，例如 `5F03` 的 `StepSec`，
  其正確性有現場步階倒數畫面逐秒佐證。
- `0FC1` 的 `HardwareStatus` 定義為 2-byte 無號整數（協定 4-22）。

> 協定 4-36 明載「`0F04` 的 HardwareStatus 各位元為建議及參考內容，
> 主辦機關可依需求修訂」。若中央端位元定義與上表不同，需取得正式對照表核對。

## 十天內控制器回報過的全部值

58,660 筆 `0F04` / `0FC1`（2026-08-28 ~ 09-08）只出現過四種值：

```
0x4000  34,623 筆 (59.02%)  bit14
0x6000  23,924 筆 (40.78%)  bit13 + bit14
0x4004      73 筆 ( 0.12%)  bit2 + bit14
0x6004      40 筆 ( 0.07%)  bit2 + bit13 + bit14
```

bit1、bit5、bit6 **從未為 1** —— 控制器未曾回報過那三項故障。

---

## 現行設定（已鎖定，不要更動）

使用者 2026-09-08 決定：**「這個不是暫時，重啟都必須維持」**、**「固定這樣，不要動」**。

```
hwstatus_mode = raw     純通透(2026-09-09 14:48 定案,取代 09-08 的 swap)
hwstatus_mask = 0x2000  遮掉 bit13(外部時相控制進行中)
```

三層已同步一致(2026-09-09 14:48 實地確認):設定檔 `raw` / 執行期 `raw` /
systemd drop-in `raw`。

兩層保險：

**第一層 — 持久化設定檔**（畫面上切了就留住，不必改 systemd、不必重啟）

```json
config/system/signal_conn.json
{ ..., "hwstatus_mode": "raw", "hwstatus_mask": 8192 }
```

**第二層 — systemd drop-in**（設定檔遺失或換機部署時兜底）

```
/etc/systemd/system/traffic-signal.service.d/zz-hwmode.conf
Environment=SIGNAL_TC3_HWSTATUS_MODE=raw
Environment=SIGNAL_TC3_HWSTATUS_MASK=8192
```

取值順序：**持久化 > env > 預設 `raw`**。`force` 模式**不持久化**（測試用，
留到重啟之後會很難查）。

執行期切換（立即生效，不必重啟）：

```bash
# 只在執行期生效(預設) —— 臨時測試用,重啟會回到設定檔的值
curl -X POST --cookie "tvd_session=$TOK" \
  ".../api/signal/control/hwstatus-mode?mode=swap&mask=8192"

# 永久改(寫進設定檔) —— 要明確帶 persist=1
curl -X POST --cookie "tvd_session=$TOK" \
  ".../api/signal/control/hwstatus-mode?mode=swap&mask=8192&persist=1"
```

🛑 `persist` 預設 **0**。臨時測試不該有能力改掉常設組態 —— 2026-09-08
就是因為預設會落檔，一次「試一下」把鎖定的 `swap` 換成了 `raw`。

模式：`raw`（純通透）/ `swap`（對調位元組）/ `flip14`（只翻 bit14，退路）/
`zero`（全報正常）/ `force`（測試指定值）。`mask` 參數設定要遮掉的位元。

---

## 🛑 機箱門開啟：`raw` 模式下要放在**線路上的 bit1**（2026-09-11）

機箱門磁開關觸發時，我方要在送給中央的 HardwareStatus 疊上「機箱門開啟」。
我方語意是 **bit9**，但在 `raw` 模式下照 bit9 送出去，中央反讀後會落在 **bit1**，
畫面顯示 **`記憶體異常`** —— 現場當天就是這樣看到的（「記憶體故障，hawcod 看一下」）。

因為現行模式是 `raw`（不做交換），要讓中央顯示正確的「機箱門開啟」，
我方必須**主動把這個位元放在線路上的 bit1**，讓中央反讀之後落回 bit9：

```
我方語意 bit9 機箱門開啟
  └─ raw 模式 → 線路上放 bit1  →  中央反讀 → bit9 → 顯示「機箱門開啟」  ✅
  └─ 若照 bit9 放 → 中央反讀 → bit1 → 顯示「記憶體異常」                ❌
```

程式碼（`api/routes/signal_tc3.py`）：

```python
CABINET_BIT = 9            # 我方語意（畫面、內部紀錄用這個）
CABINET_BIT_ON_WIRE = 1    # raw 模式下實際放到線路上的位置
...
hs |= (1 << (CABINET_BIT_ON_WIRE if mode == "raw" else CABINET_BIT))
```

`swap` 模式維持用 `CABINET_BIT`（交換那一步會自己把它搬到 bit1），
兩種模式最後上線路的位元組是同一個，這點由 `tests/test_tc3_center_relay.py` 驗。

### 定案的兩個值（已報備中央／廠商，2026-09-11）

| 情境 | 我方送 | 中央反讀 | 中央顯示 | ERROR |
|---|---|---|---|---|
| 平時 | `0x4000` | `0x0040` | 號誌驅動單元 | 不亮 |
| 機箱門開啟 | `0x4002` | `0x0240` | 號誌驅動單元＋機箱門開啟 | 不亮 |

`0x4002` 的 bit1 **是刻意放的**，不是記憶體故障。網頁上 `HardwareStatus`
欄位的解碼說明看到這兩個值會直接標註（commit `ad5e14a`），
避免日後又被當成真故障處理。

🛑 上傳中央的紀錄若出現這兩個值以外的組合，才需要當成事件追。

---

## 遮蔽 bit13（外部時相控制進行中）

`bit13` 是**狀態指示，不是故障**。只要我方持有時相控制它就恆亮，中央端顯示成
`TIMING_PLAN_ON_TRANSITION` 並當成異常在管理 —— 等於每次動態控制都在中央刷一條
不需要處理的訊息。使用者 2026-09-08 授權遮蔽。

**這不是隱瞞運轉狀態。** 中央每 5 秒輪詢 `5F40` 查控制策略，我方**據實**轉答
`0x14`（含 bit4 時相控制）——「誰在控」的權威來源完全沒有被動過。
我方只是不讓同一件事在硬體狀態欄再跳一次告警。

🛑 **白名單只允許遮 bit13 / bit14 兩個狀態指示位元。** 帶到任何錯誤類位元
（bit0–12、bit15）一律回 400：遮錯誤類等於對主管機關謊報故障情形，本專案不做。
這條線寫死在 `control_hwstatus_mode`，不是靠人記得。

遮蔽在**我方位元語意下、交換之前**套用 —— 交換之後位置就變了。

```
0x6000 遮 bit13              → 0x4000     其他位元不動
0x6004 遮 bit13              → 0x4004     bit2 保留
0x6000 遮 bit13 + swap + 機箱門 → 0x0042
```

---

## ⚠ 中央端修好之後要做什麼

正解是中央端讓**兩條路徑用同一種位元組順序**（依協定應為 big-endian）。

🛑 中央修好之後**必須同步切回 `raw`** —— 兩邊同時「修正」會再次錯開，
方向相反。切換當下用 XML 端點量一次確認。

🛑 但驗證時**不能只看 XML** —— 它沒有「硬體狀態」那個欄位。
兩個都要看：XML 的 `eq_hw_status`，以及中央畫面上的硬體狀態燈號。

```bash
curl http://10.203.6.73/api/smg/r24a/device/scmXMLData.xmL
```

`mask = 0x2000` 可以考慮拿掉，讓中央看得到「外部時相控制進行中」；
但要先確認中央不會把它當成異常在管理。

驗證方式：關閉動態控制時 bit13 應熄滅、中央對應顯示同步變化；
開啟機箱門時中央應顯示「機箱門開啟」而非「記憶體異常」。

🛑 若日後有人想改回 `swap`，先讀上面的「兩條路徑」—— `swap` 只會把問題
   從告警欄搬到故障燈，兩者不可能同時對。2026-09-08 與 09-09 兩天各切過一次，
   兩種抱怨都聽過了；現行是 `raw`（燈號綠 + 假告警），這是**使用者選定的取捨**，
   不要再自行切換。

---

## 實作位置

| 檔案 | 內容 |
|---|---|
| `api/routes/signal_tc3.py` `_hw_for_center()` | 純函式：依模式算出要送中央的值 |
| 同檔 `_forward_controller_frame_to_center()` | 轉發時套用，改完用 `build_frame` 重算 CKS + byte stuffing |
| 同檔 `_latest_hwstatus()` | 畫面顯示的「收 → 送」，與轉發共用同一支 |
| `POST /api/signal/control/hwstatus-mode` | 執行期切換；`persist=1` 才存檔（預設只在執行期生效） |
| `tests/test_tc3_control_guard.py` | 逐模式驗實際送出值、可逆性、持久化 |

改動只碰 `0F04` / `0FC1` 的那 2 個位元組，其餘所有訊息一律原封轉發；
組框若拋例外則退回原封轉送（fail-safe）。我方紀錄保留兩份：
`signal_frames.raw` 是控制器原值，`sent_hw` 是實際上線路的值。

---

## 🛑 踩過的坑

**1. `swap` 第一版掉了 bit14（曾上線路約 20 秒）**

新增 `swap` 時只在最後補了交換那一步，**沒有給它自己的取值分支** ——
`swap` 掉進 `else`（flip14），先把 bit14 XOR 掉才交換：

```
0x6000 ^ 0x4000 = 0x2000 → |bit9 = 0x2200 → 交換 = 0x0022   ← 送出去的錯誤值
```

當時的單元測試只驗**交換算術**（`swap(0x6200) == 0x0062`），沒驗**模式分派**，
所以擋不住 —— 交換函式本身是對的，錯在根本沒走到它該走的分支。
是現場 dry run 抓到的。

**教訓**：新增模式時要驗「這個模式實際送出什麼」，不是只驗它用到的那個運算。
已抽成純函式 `_hw_for_center` 並逐模式測。

**2. 位元組交換必須是最後一步**

上面所有位元運算（機箱 bit9、flip14）都是在**我方的位元語意**下做的。
先交換再設位元會設到錯的位置。

**3. `raw` 模式下遮蔽位元被靜靜忽略（曾送錯上線路）**

轉發的修改區塊條件原本是：

```python
if ((_mode != "raw" or _cab) and ...):   # raw 且機箱門關 → 整段跳過
```

所以 `raw` 模式下 `mask` 完全沒作用，控制器的 `0x6000` 原封送出
（`sent_hw=None`），中央反讀成 `0x0060` = `I/O unit error` + `SIGNAL_DRIVER`。

**後果**：那次 `raw` 測試完全無效 —— 送出去的值根本不是以為的 `0x4000`，
不能用來判斷中央改了讀法沒有。而且一度被誤判成「閂鎖的舊告警」。

**教訓**：遮蔽是**獨立於位元組順序**的設定，兩者不可以互相決定要不要套用。
條件已補 `or _msk`，並加測試釘住。

**4. 模式切換會落檔，臨時測試覆蓋了鎖定值**

現場說「試一下」，切到 `raw` 之後它**直接寫進 `signal_conn.json`**，
覆蓋掉前一天才鎖定的 `swap` —— 一次臨時測試就把常設組態換掉，
而且沒有任何地方提醒。

**已改**：`persist` 預設 **0**（只在執行期生效），要永久改必須明確帶
`persist=1`；回應會講清楚有沒有存檔、重啟後會回到哪個值。

**5. `force` 模式會被機箱位元污染**

`force` 之後仍會 OR 上機箱 bit9（在我方位元語意下），所以拿 `force` 當
`swap` 的 dry run 會得到 `0x0262` 而不是 `0x0062`。要驗 `swap` 就直接開 `swap`，
它有正確的順序，且執行期可一行切回 `raw`（不必重啟，下一框約 2 秒生效）。

---

## 中央端資料來源（往後查證都用它）

```
http://10.203.6.73/api/smg/r24a/device/scmXMLData.xmL
```

```xml
<file_attribute file_name="1min_sig_operation_data_center.xml"
                control_center_id="40" time="2026-09-08 18:01:00">
  <sig eqId="SIG-N8-E-9-L-E-1" eq_comm_status="0" eq_hw_status="" message=""/>
```

| 欄位 | 意義 |
|---|---|
| `eqId` | 中央對本路口的設備編號 `SIG-N8-E-9-L-E-1` |
| `control_center_id` | `40` |
| `eq_comm_status` | 通訊狀態，`0` = 正常 |
| `eq_hw_status` | 硬體狀態告警名稱（空 = 無告警） |

🛑 這是 **1 分鐘週期**的檔案，改值之後最多要等 60 秒才會反映。
先前幾次「看起來沒反應」很可能就是沒等滿一個週期。

🛑 **它不包含中央畫面上的「硬體狀態」燈號** —— 那是另一條路徑（正常讀 bit14）
算的。只看這份 XML 會漏掉一半的資訊，本文件就曾因此下錯結論。

## 相關

- 書面說明（可轉交中央端）：本目錄同名內容之網頁版
- `CLAUDE.md` — 專案規範
- 2026-09-07 已移除的 `flip14` 補償：同樣是憑推測改轉送資料，實證會害中央顯示異常
