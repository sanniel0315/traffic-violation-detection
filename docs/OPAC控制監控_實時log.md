...
**建議**:中心端檢查 icagent→控制器實體通訊(RS-232／MiiNePort 10.200.40.222／
...
`_by_addr` 恆空。`SIGNAL_TC3_HOST=10.200.40.222` 有設、MiiNePort 也連得到
...
## 影子模式首批結果(2026-09-02 20:18, 84 筆)

**一致率 73%**(77 筆列入,7 筆為切換當下已排除)。

**不一致的模式高度一致,且我方判斷在延滯上較優**:
```
20:17:13 綠相1(上匝道)已亮35s 排隊1=0m 排隊2=37.5m → 我方SWITCH / OPAC KEEP
20:17:18 已亮40s  排隊1=0   排隊2=31.0m           → 我方SWITCH / OPAC KEEP
20:17:23 已亮46s  排隊1=0   排隊2=37.5m           → 我方SWITCH / OPAC KEEP
20:17:28 已亮51s  排隊1=0   排隊2=37.5m           → 我方SWITCH / OPAC KEEP
20:17:33 已亮56s  排隊1=11.5 排隊2=37.5m          → 我方SWITCH / OPAC KEEP
         (OPAC 直到此後才切)
```
**上匝道綠燈亮著卻一台車都沒有(排隊 0m),下匝道積 37.5 公尺在等紅燈,
OPAC 續綠到 55 秒才切。** 我方模型判斷應提早切換 —— 綠側價值 0、
紅側延滯已達 245~299,遠超換相成本 12。

判讀:這不是我方演算法錯,而是**找到 OPAC 的一個可改善點**(空放行)。
若我方接管,這段時間的延滯可明顯降低。需累積更多樣本並用
/api/signal/shadow/outcome 的成效指標(總延滯/排隊/回堵次數)量化差距。
...
## 2026-09-03 06:4x 修「一台車被算成兩台」(使用者現場回報)

**現象**:cam3 顯示「車輛 2、排隊 11.5m」但畫面只有一台車。

**實測證據**:連續取樣抓到
```
track 323 vs 325: IoU=0.93、中心只差 3px、框寬 185
  → 同一台車被算成兩台,排隊從 ~5m 灌成 11.5m
```

**根因**:`VehicleTracker.update()` 只解決「偵測↔track」配對,
不處理「偵測本身重複」。YOLO 同一幀對同一台車吐出兩個高度重疊的框時
(NMS 沒濾掉):第一個框配到 track A → 第二個框因 A 已在 matched_tracks
而**新建 track B** → 兩 track 並存 → 車輛數×2、排隊×2。

**影響不只顯示**:排隊長度正餵給 OPAC 做號誌配時決策,灌大的排隊會讓它
誤判該給多少綠燈。與稍早的 null bug 同屬「供料品質」問題。

**修正(7693f74)**:update() 入口先 _dedupe_detections() ——
同類別、IoU>=0.7 視為重複,保留面積較大者。門檻 0.7 保守:
相鄰前後車 IoU 遠低於此,NMS 漏掉的重複框實測 0.9 以上。
只比同類別,避免把車上載物或不同車種誤併。7 個測試涵蓋各情境。
